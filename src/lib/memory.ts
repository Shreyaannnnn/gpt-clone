...
export async function recallMemory(conversationId: string): Promise<MemoryContext> {
	const convo = await (Conversation as any).findById(conversationId).lean().exec();
	if (convo?.summary) return { text: convo.summary };
	// Build a tiny summary from recent user messages as a fallback
	const lastUser = await (Message as any).find({ conversationId, role: "user" })
		.sort({ createdAt: -1 })
		.limit(3)
		.lean()
		.exec();
...
export async function storeMemory(conversationId: string, content: string) {
	...
	await (Conversation as any).findByIdAndUpdate(conversationId, { summary: content.slice(0, 800) }).exec();
}
